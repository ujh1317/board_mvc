...
public class FaqDAO {
   ...
   private static FaqDAO dao=new FaqDAO();//객체 생성
   ...
   // 원글,답글 쓰기
   //-------------------
   public void insertArticle(FaqDTO dto) throws Exception{
      int num=dto.getNum();
      int ref=dto.getRef();
   ...
   public List getList(int start,int cnt) throws Exception{
      ...
      try{
         ...
         if(rs.next()){
            list=new ArrayList<FaqDTO>();//객체 생성
            do{
               FaqDTO dto=new FaqDTO();//객체생성
   ...
   // 글 내용기
   //-------------------
   public FaqDTO getArticle(int num) throws Exception{
      FaqDTO dto=null;
   ...
   // 글수정,jsp보낼것
   //-----------------
   public FaqDTO updateGetArticle(int num) throws Exception{
      FaqDTO dto=null;
      try{
         con=getCon();//커넥션 얻기 
         pstmt=con.prepareStatement("select * from faq where num=?");
   ...
   // DB의 데이터 수정
   //---------------------
   public int updateArticle(FaqDTO dto) throws Exception{
      String dbpw="";
      int x=-10;
      try{
         con=getCon();
